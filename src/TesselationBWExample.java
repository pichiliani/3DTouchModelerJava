...
//*** and its transformation group.               ***
    ...
    Shape3D x = platform.getShape(0);
    
    
    TriangleStripArray g = (TriangleStripArray) x.getGeometry();
    ...
    g.setCapability(Shape3D.ALLOW_GEOMETRY_READ);
    g.setCapability(Shape3D.ALLOW_GEOMETRY_WRITE);
    
    
    
    float[] vertices = g.getCoordRefFloat();
    
    for(int i = 0 ; i< 3;i++)
    {
    	System.out.println("Coord X: " + String.valueOf(vertices[i])  );
    	
    	// vertices[i] = vertices[i] + 1f; 
